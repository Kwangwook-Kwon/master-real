...
int main()
{
    ...
    while (1)
    {
        ...
        if (g_recv_rate > 0)
            fprintf(fp, "%02d%02d%02d.%06ld, %f, %f, %ld, %ld ,%d, %f\n", ptm->tm_hour, ptm->tm_min, ptm->tm_sec, val.tv_usec, g_recv_rate, g_send_rate, g_rtt_app, g_rtt_hw, ack_queue_tail - ack_queue_head, g_normalize_gradient);
        sleep_cnt++;
        if (sleep_cnt > 10000)
        {
